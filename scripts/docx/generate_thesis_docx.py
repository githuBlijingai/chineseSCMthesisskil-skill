...
def add_markdown_table(doc: Document, lines: list[str], styles: dict[str, Any]) -> None:
    rows = []
    for row in lines:
        cells = [clean_table_cell(cell.strip()) for cell in row.strip().strip("|").split("|")]
        # 放宽分隔行匹配，允许更多空白字符
...
def build_doc(
    ...
    *,
    ...
    while index < len(lines):
        ...
        # 支持多种图片标记格式
        image_label = None
        for pattern in [SCREENSHOT_PLACEHOLDER_RE, re.compile(r"^@image:(.+?)$"), re.compile(r"^!\[.*?\]\((.+?)\)$")]:
            match = pattern.match(stripped)
            if match:
                image_label = match.group(1).strip()
                break
        
        if image_label:
            image_path = image_map.get(image_label)
            if image_path and image_path.exists():
                add_image(doc, image_path)
            else:
                add_missing_asset_placeholder(doc, image_label, styles)
            seen_content = True
            index += 1
            continue
        ...
        # 清理行首空格和特殊字符后检测表格
cleaned_line = stripped.lstrip().replace('\ufeff', '').replace('↓', '')
if cleaned_line.startswith("|"):
            table_lines = []
            while index < len(lines):
                line_cleaned = lines[index].strip().lstrip().replace('\ufeff', '').replace('↓', '')
                if not line_cleaned.startswith("|"):
                    break
