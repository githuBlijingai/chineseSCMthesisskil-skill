...
TECH_MARKERS = {
    ...
    # Embedded/MCU markers
    "platformio.ini": "PlatformIO / Embedded",
    "arduino.json": "Arduino",
    "stm32cubeide.project": "STM32CubeIDE",
    "CMakeLists.txt": "CMake (possibly Embedded)",
    "Makefile": "Make (possibly Embedded)",
    "build.json": "Espressif/ESP-IDF",
}
...
# Hardware/PCB file hints (for MCU projects)
HARDWARE_HINTS = ("pcb", "hardware", "schematic", "bom", "gerber", "altium", "kicad", "嘉立创", "立创")
HARDWARE_EXTS = {
    ".pcbdoc", ".prjpcb", ".kicad_pcb", ".brd",  # PCB
    ".schdoc", ".sch", ".dsn", ".kicad_sch",     # Schematic
    ".gtl", ".gbl", ".gbs", ".gbo", ".drl",       # Gerber
}
...
class Evidence:
    ...
    # Embedded/MCU specific
    embedded_source_files: list[str] = None  # .c, .cpp, .h, .hpp, .ino
    hardware_files: list[str] = None  # PCB, schematic, BOM
...
def build_evidence(root: Path) -> Evidence:
    ...
    for path in files:
        ...
        # Detect embedded/MCU source files
        if path.suffix.lower() in {".c", ".cpp", ".h", ".hpp", ".ino", ".asm", ".s"}:
            embedded_source_files.append(r)
        # Detect hardware files
        if any(term in lower for term in HARDWARE_HINTS):
            hardware_files.append(r)
        if path.suffix.lower() in HARDWARE_EXTS:
            hardware_files.append(r)
        # BOM files
        if path.suffix.lower() in {".csv", ".xlsx", ".xls", ".txt"}:
            if "bom" in lower or "物料" in lower or "器件" in lower:
                hardware_files.append(r)
    ...
    return Evidence(
        ...
        embedded_source_files=embedded_source_files[:500],
        hardware_files=hardware_files[:200],
    )
...
def write_outputs(evidence: Evidence, out: Path) -> None:
    ...
    # Embedded/MCU specific outputs
    if evidence.embedded_source_files:
        write_list_md(
            out / "embedded-source.md",
            "Embedded/MCU Source Code Evidence",
            evidence.embedded_source_files,
            "No embedded source files detected.",
        )

    if evidence.hardware_files:
        write_list_md(
            out / "hardware-design.md",
            "Hardware Design Evidence (PCB/Schematic/BOM)",
            evidence.hardware_files,
            "No hardware design files detected.",
        )
...
def main() -> int:
    ...
    if evidence.embedded_source_files:
        print(f"Embedded source files: {len(evidence.embedded_source_files)}")
    if evidence.hardware_files:
        print(f"Hardware design files: {len(evidence.hardware_files)}")
        print("Hint: For detailed hardware analysis, run:")
        print("  python scripts/evidence/build_hardware_evidence.py --project-path .")
    return 0
