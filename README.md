...
##更多skills请查看https://www.meikeit.com/makerclan/skill-center。我也会同步上线在这个市场
...
**chineseSCMthesisskil-skill** 不是一键生成论文的工具。它是一套论文标准化交付工作台，把毕业论文拆成可审查、可回退的步骤——收集材料 → 选择交付路径 → 构建证据 → 确认大纲 → 分章写作 → 生成/填充/编辑 DOCX → 质量检查。
...
- **单片机/嵌入式系统设计论文**（STM32、51单片机、Arduino等）
...
# 3. 解析硬件设计文件（PCB/原理图/BOM）
python scripts/evidence/build_hardware_evidence.py --project-path .

# 4. 生成论文（使用单片机专用章节配置）
python scripts/docx/build_complete_thesis.py --spec thesis-ai-standard/templates/thesis-ai-spec.yaml --body paper-output/thesis-body.md --output paper-output/
```

**支持的硬件设计文件：**
| 类型 | 格式 |
|------|------|
| PCB设计 | `.pcbdoc` (Altium), `.kicad_pcb` (KiCad), `.brd` (Cadence/Eagle) |
| 原理图 | `.schdoc` (Altium), `.sch` (通用), `.dsn` (Cadence) |
| BOM物料清单 | `.csv`, `.xlsx`, `.txt` |
| Gerber文件 | `.gtl`, `.gbl`, `.drl` |

**单片机论文典型章节结构：**
- 第2章 系统方案与关键技术（芯片选型对比）
- 第3章 硬件电路设计（主控/电源/传感器/通信/PCB）
- 第4章 软件程序设计（开发环境/主程序/模块/中断/协议）
...
# 5. 在 Claude Code 中加载本 skill，告诉 AI：
...
```powershell
# CNKI文献检索
python scripts/literature/build_cnki_pool.py --spec thesis-ai-standard/templates/thesis-ai-spec.yaml --output paper-context/literature/
...
| 项目证据提取    | 按论文类型从源码、数据库、API、测试、实验数据、硬件设计文件（PCB/原理图/BOM）中生成证据索引    |
...
治理侧 paper-context/
  ...
  evidence/         项目证据、技术栈、数据库 Schema、API 列表
                    （含 hardware-evidence.json/md - 硬件证据）
                    （含 embedded-source.md - 嵌入式源码索引）
  literature/       参考文献抽取、交叉引用、联网检索建议
  standards/        标准解析、样式提取（含 chapter-profile-mcu.yaml）
  aigc/             AIGC 风格报告
...
chinese-thesis-workbench/
  ...
  scripts/                 可执行脚本
    ...
    evidence/              项目证据构建（含 parse_hardware_files.py, build_hardware_evidence.py）
...
| 单片机/嵌入式论文支持         | 不支持                  | ✅ 完整支持（硬件证据解析、章节配置、写作指导）           |
| 硬件设计文件解析              | 不支持                  | ✅ 支持 PCB/原理图/BOM/Gerber               |
...
- **新增** `scripts/evidence/parse_hardware_files.py`（747 行）—— 硬件设计文件解析器
- **新增** `scripts/evidence/build_hardware_evidence.py` —— 硬件证据自动构建脚本
- **新增** `HARDWARE_EVIDENCE_USAGE.md` —— 硬件证据使用指南
- **修改** `scripts/docx/generate_thesis_docx.py` —— 新增 `sample_style_override` + `--sample-analysis` 参数
- **修改** `scripts/evidence/build_project_evidence.py` —— 添加嵌入式源码和硬件文件检测
- **修改** `assets/.../thesis-ai-spec.yaml` —— 添加 hardware_sources 和 embedded_code 配置
- **修改** `assets/.../chapter-profile-mcu.yaml` —— 更新硬件证据绑定要求
- **修改** `SKILL.md` —— 4 处更新（交付路径、格式例外、硬规则、资源映射）+ 硬件证据说明
- **修改** `references/workflow/intake.md` —— 新增四选一交付路径问题 + 硬件文件需求检查
- **修改** `references/writing/writing-pipeline.md` —— 添加单片机论文写作指导
- **修改** `references/delivery/final-delivery-check.md` —— 添加单片机交付检查清单
- **修改** `references/evidence/source-to-thesis-workflow.md` —— 添加硬件证据工作流
...
- **修改** `MCU_PROFILE_USAGE.md` —— 添加硬件证据故障排除
- **修改** `MODIFICATION_SUMMARY.md` —— 完整记录所有修改
