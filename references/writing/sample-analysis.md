...
1. **`template_rules`（最高优先级）**：学校模板、学院规范、导师要求、任务书、开题报告中可确认的规则。特别是当用户上传的模板中有**清晰的论文大纲/章节结构**时，必须**优先使用该大纲**，不得用skill默认章节结构覆盖。
2. **`sample_patterns`**：往届样文中观察到的结构、节奏和表达习惯。它们只能作为参考。
3. **`recommendations`**：仅当用户未提供明确大纲或大纲不完整时，才结合当前论文类型和默认规则生成建议。它们必须等待用户确认后才能进入正式目录或字数预算。

**大纲使用规则**：
- 如果解析结果显示用户模板中有完整的章节列表（如第1章...第6章），直接在`outline_suggestion`中复现该结构，标注来源为"用户模板"
- 如果用户模板大纲与skill默认推荐差异较大，以用户模板为准，在报告中说明"按用户提供的模板大纲撰写"
- 仅当用户模板大纲缺失、模糊或不完整时，才提供skill默认的章节建议
